...
def GetEEGPartitionNumber(EEGpath: str, 
    ...
    Return a Pandas DataFrame with the exact number of samples which can be extracted 
    from each EEG file in the EEGpath directory
    
    
    Parameters
    ----------
    EEGpath : string 
        Directory with all EEG files.
    freq : int or float 
...
EEGlen=GetEEGPartitionNumber('Data_Folder/', freq, DeltaT, overlap=overlap, includePartial=True, save=False)
EEGlen


# # Function to create the train-validation/test split table
...
def get_subarray_closest_sum(arr: Sequence[int], 
    ...
    To find the subarray get_subarray_closest_sum, try multiple times subarray_closest_sum, 
    shuffling the array at each try. If the array has the sum closest to the target up to the given 
    tolerance, it stops the search.
...
def GetEEGSplitTable(partition_table: pd.DataFrame=None,
    ...
    if save:
        EEGspli.to_csv('EEGTrainTestSplit.csv')        
...
dataset_info= pd.read_excel('DATASET_INFO.xlsx')[['dataset_number_reference', 'dataset_name']]
EEGsplit= GetEEGSplitTable( test_ratio=0.2, val_ratio=0.2, partition_table = EEGlen,  dataset_info=dataset_info,
                           test_split_mode=1,val_split_mode=2 )
EEGsplit

print(EEGlen.loc[ EEGlen['file_name'].isin( EEGsplit.loc[EEGsplit['split_set']==0, 
                                                                 'file_name'].values), 'N_samples'].sum())
print(EEGlen.loc[ EEGlen['file_name'].isin( EEGsplit.loc[EEGsplit['split_set']==1, 
                                                                 'file_name'].values), 'N_samples'].sum())
print(EEGlen.loc[ EEGlen['file_name'].isin( EEGsplit.loc[EEGsplit['split_set']==2, 
                                                                 'file_name'].values), 'N_samples'].sum())
...
fakeEEGlen = pd.DataFrame(fakeEEGlen, columns=['file_name', 'N_samples'])
dataset_info= pd.read_excel('DATASET_INFO.xlsx')[['dataset_number_reference', 'dataset_name']]
start=time.time()
EEGsplit=GetEEGSplitTable(EEGpath =None, 
                          test_ratio= 0.3,  
                          val_ratio=0.4,
                          test_data_id=None,#{10: [2], 1: [1,2]}, 
                          val_data_id=None,#{2:[1,2], 3:[2,3]}, 
                          exclude_data_id=[10],
                          partition_table = fakeEEGlen,
...
print(fakeEEGlen.loc[ fakeEEGlen['file_name'].isin( EEGsplit.loc[EEGsplit['split_set']==0, 
                                    'file_name'].values), 'N_samples'].sum()/fakeEEGlen['N_samples'].sum())
print(fakeEEGlen.loc[ fakeEEGlen['file_name'].isin( EEGsplit.loc[EEGsplit['split_set']==1, 
                                    'file_name'].values), 'N_samples'].sum()/fakeEEGlen['N_samples'].sum())
print(fakeEEGlen.loc[ fakeEEGlen['file_name'].isin( EEGsplit.loc[EEGsplit['split_set']==2, 
                                    'file_name'].values), 'N_samples'].sum()/fakeEEGlen['N_samples'].sum())
...
class EEGDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        start=(self.Nsample-round(self.Nsample*overlap))*(partition)
        ...
        if self.supervised:
            if self.label_on_subj_info:
                label = self.subj_info[list(set(self.subj_info_keys).intersection(self.subj_info_key))[0]]
            else:
...
EEGlen=GetEEGPartitionNumber('Data_Folder/', freq, DeltaT, overlap=overlap, includePartial=True, save=False)
EEGsplit= GetEEGSplitTable( test_data_id=[1,2], val_data_id=4, partition_table = EEGlen)

EEGData=EEGDataset( 
                 EEGpath='Data_Folder/', 
                 EEGlen=EEGlen, 
                 EEGsplit = EEGsplit, 
                 EEGpartition_spec = [freq, DeltaT, overlap] ,
                 mode='train',
                 supervised=True,
                 label_on_subj_info=True,
                 subj_info_key=['participant_id','part'],
                 get_label_fun=None,
                 optional_fun_args=None
                )
a,b=EEGData.__getitem__(0)
a,b=EEGData.__getitem__(759)
...
class EEGsampler(Sampler):
    ...
    def __iter__(self):
        ...
        # -------------------------------------------------------------
        # 1st - shuffle partitions of the same subject for each subject 
        # -------------------------------------------------------------
        for ii in range(self.Nsubject-1):
            random.seed(Nseed)
        ...
        # ------------------------------------------------------------
        # 2nd - Arrange index According to batch and number of workers
        # ------------------------------------------------------------
        ...
        #iterator=iter(range(len(self.data_source)))
...
batch=16
worker=8
EEGsplit= GetEEGSplitTable(val_ratio=0, test_data_id=[1], val_data_id=None, partition_table = EEGlen)
EEGData=EEGDataset( EEGpath='Data_Folder/', EEGlen=EEGlen, EEGsplit = EEGsplit, 
                   EEGpartition_spec = [freq, DeltaT, overlap] ,mode='train',
                 supervised=True,label_on_subj_info=True,
                 subj_info_key=['participant_id','part'],get_label_fun=None, optional_fun_args=None
                )
SamplerEEG= EEGsampler(EEGData, batch, worker)
for i in SamplerEEG:
    print(i)
...
BatchSize=16
Workers=0
EEGsplit= GetEEGSplitTable(val_ratio=0, test_data_id=[5], val_data_id=None, partition_table = EEGlen)
EEGData=EEGDataset( EEGpath='Data_Folder/', EEGlen=EEGlen, EEGsplit = EEGsplit, 
                   EEGpartition_spec = [freq, DeltaT, overlap] ,mode='train',
                 supervised=False,label_on_subj_info=True,
                 subj_info_key=['participant_id','part'],get_label_fun=None, optional_fun_args=None)
SamplerEEG= EEGsampler(EEGData, batch, Workers)

dataloader= DataLoader(EEGData, batch_size=BatchSize, sampler=SamplerEEG, num_workers=Workers)
...
print("--- code executed in %s seconds ---" % (time.time() - Tstart))
