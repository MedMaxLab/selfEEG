...
class TestAugmentationFunctional(unittest.TestCase):
    ...
    def test_shift_vertical(self):
        ...
        aug_args = { 'x': [self.x1, self.x2, self.x3, self.x4,
                           self.x1np, self.x2np, self.x3np, self.x4np], 'value': [1, 2.0, 4] }
        aug_args = self.makeGrid(aug_args)
        for i in aug_args:
    ...
    def test_moving_average(self):
        ...
        aug_args = { 'x': [ self.x1, self.x2, self.x3, self.x4,
                            self.x1np, self.x2np, self.x3np, self.x4np], 'order': [3, 5, 9] }
        aug_args = self.makeGrid(aug_args)
        for i in aug_args:
    ...
    def test_permute_channels(self):
        ...
        aug_args = self.makeGrid(aug_args)
        for i in aug_args:
            i['x'] = i['x'] + (torch.randn(32,1) if isinstance(i['x'], torch.Tensor) else np.random.randn(32,1))
